...
1. **Haptic Left Hand** $HH$ calculated after **Visual Wall** $VW$ calculation done:
    ...
    HH = \begin{cases}
        AH & \text{if } |AH - HW| > r \\
        HW + (AH - HW) \cdot r & \text{otherwise}
    \end{cases}
    $$
    
    3. After **Haptic Left Hand** $HH$ calculation done, distance between **Actual Left Hand** $AH$ and **Haptic Left Hand** $HH$ will be sent to my device to display force based on its magnitude.
    $$ Force =  |HH - AH| $$
